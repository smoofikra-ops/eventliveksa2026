...
pattern = re.compile(
    r'<ul className="flex items-center gap-2 md:gap-4" onMouseLeave=\{\(\) => setHoveredIndex\(null\)\}>\s*\{navLinks\.slice\(0, 2\).*?layoutId="mercury-desktop-nav".*?transition=\{\{.*?\}\}\s*/>\s*\)\}\s*</li>\s*\)\}\)\}\s*</ul>',
    re.DOTALL
)
