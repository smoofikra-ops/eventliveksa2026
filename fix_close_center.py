...
new_services_close = """              <button 
                onClick={(e) => { e.stopPropagation(); setSelectedServiceIndex(null); }} 
                className="fixed top-8 left-1/2 -translate-x-1/2 z-[9999] flex items-center justify-center gap-2 px-6 py-3 bg-black/90 hover:bg-amber-500 text-white hover:text-black border border-white/40 rounded-full backdrop-blur-2xl transition-all duration-300 pointer-events-auto shadow-[0_0_30px_rgba(0,0,0,0.9)] active:scale-95 group animate-in slide-in-from-top-10 fade-in duration-500"
              >
                <X className="w-5 h-5 md:w-6 md:h-6 transition-transform group-hover:rotate-90" />
                <span className="text-sm md:text-base font-bold tracking-wide">{language === 'ar' ? 'إغلاق' : 'Close'}</span>
              </button>"""
...
print("Applied CENTERED FIXED close button updates.")
